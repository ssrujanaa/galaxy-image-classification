...
if [ ! -x Training_Preprocess2_py ]; then
    /bin/chmod +x Training_Preprocess2_py
fi

printf "\n##################### Checking file integrity for input files #####################\n"  1>&2
# do file integrity checks
pegasus-integrity --print-timings --verify=stdin 1>&2 << 'eof'
Training_labels_Preprocess1.npy:Training_Preprocess2_py:Training_images_Preprocess1.npy
eof
...
pegasus-kickstart  -n Training_Preprocess2.py -N ID0000003 -R condorpool  -s Training_images_Preprocess2.npy=Training_images_Preprocess2.npy -s Training_labels_Preprocess2.npy=Training_labels_Preprocess2.npy -L Galaxy -T 2021-04-08T19:14:50+00:00 ./Training_Preprocess2_py 
job_ec=$?
